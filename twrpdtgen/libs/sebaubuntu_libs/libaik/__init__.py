...
class AIKManager:
	...
	def __init__(self):
		...
		# Use bundled AIK instead of cloning from GitHub
		if not BUNDLED_AIK_PATH.is_dir():
			raise RuntimeError(
				f"Bundled AIK not found at {BUNDLED_AIK_PATH}. "
				"Please ensure the 'aik' directory is present in the twrpdtgen package."
			)
		copytree(str(BUNDLED_AIK_PATH), str(self.path), dirs_exist_ok=True)
